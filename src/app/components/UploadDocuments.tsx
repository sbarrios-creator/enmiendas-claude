      ...
      {groupedDocuments.map(({ category, docs }) => {
        ...
        return (
          <div key={category} className="mb-6 border border-gray-300 rounded overflow-hidden">
            ...
            {/* Documentos — scrollbar si hay más de 5 */}
            <div className={docs.length > 5 ? 'max-h-[480px] overflow-y-auto' : ''}>
                    ...
                    <div className="grid grid-cols-2 gap-4 p-4 bg-white">
                          ...
                          <div className="flex items-center gap-2">
                            <label className="flex-1 flex items-center gap-2 bg-gray-50 border border-gray-300 rounded px-3 py-2 cursor-pointer hover:bg-gray-100">
                              ...
                              <input
                                type="file"
                                className="hidden"
                                onChange={(e) => { const f = e.target.files?.[0]; if (f) handleFileSelect(doc.id, 'controlChanges', f.name); }}
                              />
                            </label>
                            <button
                      ...
                      <div>
                        <p className="text-xs font-semibold text-gray-600 uppercase mb-2">
                          Archivo versión final <span className="normal-case font-normal text-gray-400">(con cambios aceptados)</span>
                        </p>
                            ...
                            {isPending && !docFiles.controlChanges ? (
                              ...
                              <label className="flex-1 flex items-center gap-2 bg-gray-50 border border-gray-300 rounded px-3 py-2 cursor-pointer hover:bg-gray-100">
                                ...
                                <input
                                  type="file"
                                  className="hidden"
                                  onChange={(e) => { const f = e.target.files?.[0]; if (f) handleFileSelect(doc.id, 'finalVersion', f.name); }}
                                />
                              </label>
                            )}
                            <button
